  ...
  head: () => ({
    meta: [
      { title: "Contact | Senior Concierge Services in Houston | Roadrunner Express" },
      {
        name: "description",
        content:
          "Contact Roadrunner Express in Houston, TX. Call 409-455-1963 or send a message — we serve Houston and surrounding communities and reply within one business day.",
      },
      { property: "og:title", content: "Contact Roadrunner Express · Houston Senior Concierge" },
      {
        property: "og:description",
        content:
          "Serving Houston and surrounding communities. Call 409-455-1963 to schedule concierge service.",
      },
        ...
        <div className="container-luxe grid lg:grid-cols-[1.05fr_1fr] gap-14 items-end">
          <div>
            ...
            <p className="mt-8 text-lg leading-relaxed text-[var(--brand-deep)]/75 max-w-xl animate-rise delay-2">
              Serving Houston and surrounding communities. Prefer a form? Fill in a few details and
              we'll follow up personally within one business day.
            </p>
          </div>
          <div className="grid sm:grid-cols-2 gap-4 animate-rise delay-3">
            <a
              href="tel:4094551963"
              className="rounded-2xl bg-white p-6 border border-[color-mix(in_oklab,var(--brand)_10%,transparent)] hover-lift"
            >
              <Phone className="h-5 w-5 text-[var(--ember)]" />
              <p className="mt-4 text-xs uppercase tracking-widest text-[var(--brand-deep)]/60">
                Call
              </p>
              <p className="mt-1 font-display text-2xl text-[var(--brand-deep)]">409-455-1963</p>
            </a>
            <a
              href="mailto:roadrunnerexpresinc@gmail.com"
              className="rounded-2xl bg-white p-6 border border-[color-mix(in_oklab,var(--brand)_10%,transparent)] hover-lift"
            >
              ...
              <p className="mt-1 font-display text-lg text-[var(--brand-deep)] break-all">
                roadrunnerexpresinc@gmail.com
              </p>
            </a>
            <div className="rounded-2xl bg-white p-6 border border-[color-mix(in_oklab,var(--brand)_10%,transparent)]">
              ...
              <p className="mt-1 text-sm text-[var(--brand-deep)]">
                3707 Cypress Creek Pkwy, Suite 310
                <br />
                Houston, TX 77068
              </p>
            </div>
            ...
            <div className="rounded-[2rem] bg-white border border-[color-mix(in_oklab,var(--brand)_10%,transparent)] p-8 md:p-12">
              {submitted ? (
                <div className="text-center py-10">
                  <div className="mx-auto grid h-16 w-16 place-items-center rounded-full bg-[var(--ember-soft)] text-[var(--ember)]">
                    <Check className="h-8 w-8" />
                  </div>
                  <h2 className="mt-6 font-display text-4xl text-[var(--brand-deep)]">
                    Thank you!
                  </h2>
                ...
                <form onSubmit={onSubmit} className="space-y-8">
                  <div className="flex items-center justify-between">
                    <p className="eyebrow">
                      Client intake · Step {step + 1} of {total}
                    </p>
                    <div className="flex gap-1.5">
                      {Array.from({ length: total }).map((_, i) => (
                        <span
                          key={i}
                          className={`h-1.5 w-8 rounded-full transition-colors ${i <= step ? "bg-[var(--ember)]" : "bg-[var(--brand-soft)]"}`}
                        />
                      ))}
                    </div>
                  </div>
                    ...
                    <div className="grid sm:grid-cols-2 gap-5">
                      ...
                      <Field label="Home address" required>
                        <input
                          ...
                          className={inputCls}
                          placeholder="1234 Cypress Ln, Houston, TX"
                        />
                      </Field>
                    </div>
                  )}
                      ...
                      <Field label="Transportation needed?" className="sm:col-span-2">
                        <div className="flex gap-3">
                          {["Yes", "No"].map((v) => (
                            <label
                              key={v}
                              className={`flex-1 cursor-pointer rounded-2xl border px-5 py-3 text-center font-medium transition ${data.transportation === v ? "border-[var(--ember)] bg-[var(--ember-soft)] text-[var(--brand-deep)]" : "border-[color-mix(in_oklab,var(--brand)_12%,transparent)] text-[var(--brand-deep)]/70"}`}
                            >
                              <input
                                type="radio"
                                name="transportation"
                                value={v}
                                checked={data.transportation === v}
                    ...
                    <div className="grid gap-5">
                      <Field label="Additional notes">
                        <textarea
                          ...
                          className={inputCls}
                          placeholder="Tell us anything that will help us take great care of you."
                        />
                      </Field>
                  ...
                  {step === 3 && (
                    <div className="grid gap-5">
                      <Field label="Preferred contact method">
                        <div className="grid grid-cols-3 gap-3">
                          {["Phone", "Text", "Email"].map((v) => (
                            <label
                              key={v}
                              className={`cursor-pointer rounded-2xl border px-5 py-3 text-center font-medium transition ${data.contactMethod === v ? "border-[var(--ember)] bg-[var(--ember-soft)] text-[var(--brand-deep)]" : "border-[color-mix(in_oklab,var(--brand)_12%,transparent)] text-[var(--brand-deep)]/70"}`}
                            >
                              <input
                                type="radio"
                                name="contactMethod"
                                value={v}
                                checked={data.contactMethod === v}
                      ...
                      <div className="rounded-2xl bg-[color-mix(in_oklab,var(--brand-soft)_60%,var(--cream))] p-6">
                        <p className="font-display text-lg text-[var(--brand-deep)]">
                          Ready to send?
                        </p>
                        <p className="mt-2 text-sm text-[var(--brand-deep)]/70">
                          By submitting, you'll hear from a real member of our team — usually within
                          a few hours.
                        </p>
                      </div>
                    </div>
